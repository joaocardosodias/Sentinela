...
import { ShieldCheck, MapPin, Clock, X } from 'lucide-react'
import Sidebar from '../components/Sidebar.jsx'
import { getApprovedScans, resolveAssetUrl } from '../services/api.js'
...
export default function AcceptedVehicles({ onLogout, onNavigate, currentScreen, activeTourStep }) {
  ...
  useEffect(() => {
    ...
    if (activeTourStep.openMode === 'confirmed-detail' && approvedVehicles[0]) {
      setSelectedScan(approvedVehicles[0])
      return
    }

    if (selectedScan && activeTourStep.openMode !== 'confirmed-detail') {
      setSelectedScan(null)
    }
  }, [activeTourStep, approvedVehicles, selectedScan])

  async function loadApprovedScans() {
    setLoading(true)
    setError('')

    try {
      const data = await getApprovedScans()
      ...
      setError(err.message || 'Não foi possível carregar os veículos confirmados')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div style={{ display: 'flex', height: '100vh', background: C.bg }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: '32px' }}>
        ...
        {approvedVehicles.length === 0 ? (
          ...
          >
            {approvedVehicles.map((v, i) => (
              <VehicleCard
                key={`${v.plate}-${i}`}
                vehicle={v}
                onClick={() => setSelectedScan(v)}
                tourId={i === 0 ? 'confirmed-card-first' : undefined}
              />
            ))}
          </div>
        )}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered ? '0 4px 16px rgba(255,51,102,0.14)' : 'none',
      }}
    >
      <div
        data-tour="confirmed-detail"
        style={{
          height: '120px',
          background: '#F3F4F6',
          position: 'relative',
      ...
      >
        ...
        >
          <ShieldCheck size={11} color={C.pink} />
          <span
            style={{
              fontSize: '10px',
              fontWeight: '700',
...
function EmptyState() {
  return (
    ...
    >
      ...
      >
        <ShieldCheck size={28} color="rgba(255,51,102,0.4)" />
      </div>
      <p style={{ fontSize: '15px', fontWeight: '600', color: C.dark, marginBottom: '6px' }}>
        Nenhum veículo confirmado
      </p>
      <p style={{ fontSize: '13px', color: C.gray }}>
        Os veículos aceitos na Central de Alertas aparecerão aqui.
      </p>
    </div>
  )
}

function normalizeApprovedScan(scan) {
  return {
    id: scan.id,
      ...
      >
        ...
        <div style={{ display: 'flex', flex: 1, height: '100%' }}>
          ...
          >
            ...
            >
              <ShieldCheck size={11} color={C.pink} />
              <span style={{ fontSize: '14px', fontWeight: '700', color: C.pink, letterSpacing: '0.5px' }}>
                CONFIRMADO
              </span>
            </div>
          </div>
          ...
          <div style={{ flex: 1, padding: '48px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px 40px', marginBottom: 'auto' }}>
              ...
              <Field label="Validado Em" value={vehicle.acceptedAt} large />
            </div>
          </div>
        </div>
      </div>
